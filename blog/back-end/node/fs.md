---
date: 2024-07-09T04:13:20.000Z
tags:
  - backend
  - node-js
description: Node.js 文件系统模块（fs）全面指南，包括同步/异步读写、流式操作、目录操作、文件监控等。涵盖 Promise 封装和实际开发中的文件处理场景。
title: fs
authors:
  - eave
---
...
文件写入通俗来说就是将数据写入到文件中，`fs` 模块提供了[同步和异步](/blog/back-end/node/syncAsync)写入的方法。
...
文件读取通俗来说就是从文件中读取数据，`fs` 模块提供了[同步和异步](/blog/back-end/node/syncAsync)读取的方法。
